...
class SelectWithLevels extends Component {
  ...
  render() {
    ...
    const {subLabel, label, onRemove} = this.props;
    ...
    return (
      <div className={css}>
        ...
        <span style={{cursor: 'pointer'}} onClick={onRemove}>
          <i className='mb-ico-button-delete'/>
        </span>}
  ...
  handleSelectChange = (selectValue) => {
    ...
    const newVal = {...oldVal, select: selectValue && selectValue.value || ''};
  ...
  handleLevelChange = (level) => {
    ...
    const newVal = {...oldVal, level: level || ''};
