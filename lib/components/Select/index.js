...
class Select extends Component {
  ...
  static propTypes = {
    ...
    onChange: PropTypes.func,  // does NOT mutate redux-form Field data
  ...
  static defaultProps = {
    ...
    clearIconHTML: '<i class="mb-icons-cross"></i>'  // DANGER !
  };

  arrowRenderer({onMouseDown}) {
    const {arrowRenderer, noArrow} = this.props;

    if (noArrow) return <span></span>;
  ...
  onBlur() {
    ...
    if (!onBlur && typeof input.onBlur === 'function') input.onBlur(value);
  }

  render() {
    const {
      meta, input, placeholder, loadOptions, options, onChange, onSelect,
